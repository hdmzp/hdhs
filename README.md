...
│   ├── celeb_day_sweep.py                      # 셀럽PGM 공통: 편성표에서 빠진 방송 회차 보강(하루 2회 방송 대응)
...
ⓕ는 결이 다르다 — 수집은 멀쩡한데 **누적 기록이 덮여 사라지는** 사고를 잡는다(위 2026-08-22 왕톡 사고는 per-program 산출물이 전부 정상이라 ⓐ~ⓔ 어디에도 안 걸렸다). 판정은 `build_celeb_history.py`의 `record_is_final()`을 그대로 import해서 쓴다 — 누적 규칙과 검사 기준이 갈라지면 검사가 무의미해진다. **'방송 시작 = 확정'이 아니다**: 시작 후 정정 창(`RECONCILE_HOURS`) 안에서는 상품 제외·코드 변경이 정상적으로 반영되므로 그 구간의 건수 감소는 사고가 아니다.
...
python tools/test_celeb_day_sweep.py            # 셀럽PGM 회차 보강(편성표 훑기) 테스트 (네트워크 불필요)
...
- **GS/CJ/LT도 같은 가정을 깔고 간다** — `fixed/celeb_day_sweep.py`(공통 안전망)가 각 사 편성표(`{사}_live`, 이미 홈쇼핑 워크플로우가 하루 여러 번 수집해 둔 것)에서 그 프로그램의 방송을 훑어 **수집분에 없는 회차만** 채운다. 실제로 최유라쇼는 2026-09-05에 08:20/09:20/10:20 3회 방송했다. 규칙 3가지: ⑴ 이미 수집된 회차는 안 건드린다(편성표 상품명은 화면용으로 정제돼 있어 섞으면 같은 상품이 두 줄로 보인다), ⑵ **아직 시작 안 한 회차만** 채운다 — 방송이 시작되면 상세페이지가 그 회차를 목록에서 빼는데, 편성표는 슬롯당 대표상품 1개만 실어서 그걸 '빠진 회차'로 보고 채우면 20건짜리 회차가 1건으로 쪼그라든다(2026-09-04 소유진쇼 20:35 방송 12분 뒤 수집에서 실제로 발생 → 건전성 검사의 급감 경보로 잡힘). 지난 회차는 확정 기록의 영역이라 새로 만들지도 않는다, ⑶ **상품을 하나도 못 가져온 프로그램은 건너뛴다** — 죽은 스크래퍼를 편성표로 덮으면 건전성 검사가 '조용한 실패'를 못 잡는다. 회차 라벨은 회사별 표기를 그대로 따른다(누적 규칙이 라벨의 `HH:MM`으로 회차를 특정하므로)
- **반대로, 한 방송이 여러 회차로 쪼개져 보이는 것도 묶는다.** 롯데는 편성표도 상세페이지도 한 방송을 **상품 구간별로 쪼개서** 준다(2026-09-05 최유라쇼 `08:20~09:20 / 09:20~10:20 / 10:20~10:35` = 끊김 없는 한 방송. 다른 3사는 방송당 구간 1개). `merge_continuous_slots()`가 이어지는 구간을 첫 구간 시각의 한 회차로 묶는다 — ⑴ 편성표에 그 날이 있으면 **종료시각**으로 정확히(앞 구간 끝~뒤 구간 시작이 `SEGMENT_GAP_MIN`(10분) 이내), ⑵ 편성표 범위 밖(오늘+6일 이후)이면 시작시각 간격 `FALLBACK_GAP_MIN`(150분) 이내로 잠정 판단하고 방송이 가까워지면 정확히 다시 계산된다. 하루 2회 방송(오감쇼 08:15/19:30 = 675분)은 어느 기준으로도 안 묶인다. 묶인 회차의 상품에는 원래 구간을 `segment_time`(`08:20-09:20(60')`)으로 남겨 프런트가 상품 앞에 뱃지로 보여준다 — 한 회차로 보되 어느 구간에 나온 상품인지는 알 수 있다(구간이 하나뿐인 방송은 회차 라벨과 같은 말이라 안 붙인다)
- 회차가 묶이면 옛 회차 항목이 월 누적에 유령으로 남으므로, `build_celeb_history`는 **새 수집분에 없는 '방송 전' 회차를 지운다**(편성 취소로 사라진 회차도 같이 정리된다). 정정 창·확정 회차는 절대 안 건드린다
...
- **누적 규칙: 회차마다 `before → reconcile → final` 3단계.** 판정 기준이 날짜가 아니라 **시작 시각**인 게 핵심이다 — 날짜로만 보면 방송 당일 저녁 수집분이 그날 아침 방송의 기록을 덮어쓴다. 시작 시각은 `기존 기록 라벨 → 새 수집분 라벨 → schedule_raw` 순으로 찾고, 어디서도 못 읽으면 보존 쪽을 택한다
  ...
  반대로 정정 창이 확정 기록을 날리지 않도록 두 겹으로 막는다 — ⑴ **새 수집분 라벨이 회차를 시각까지 특정**해야 한다(방송이 끝나면 `8/22(토) 방송상품`처럼 시각 없는 잔여 표기로 바뀌며 다음 회차 상품이 섞여 들어온다), ⑵ 기존 기록의 **`MIN_RETENTION`(50%) 이상이 남아야** 제외로 인정한다. 2026-08-22 왕영은의 톡투게더 사고(08:34에 정상 수집된 3건이 같은 날 20:24 수집분의 잔여 노출 1건으로 교체되며 통째로 사라짐)가 ⑴에서 걸린다. 같은 상품인지는 `상품코드(링크의 /item/{코드}) → 상품명` 순으로 보므로, 코드만 바뀐 상품은 '제외+추가'가 아니라 `code_changed`로 남는다
...
- 헤더: `H로고 + 시청환경조회`(부제목 없음). 로고는 바로가기 아이콘의 H 마크에서 배경을 걷어낸 `icons/logo-h.png`
