...
- **다음 방송일은 요일 산술이 아니라 편성표가 정한다.** `pgm-comm`이 방송일시를 못 주면 예전엔 `schedule_raw`(`매주 화요일 19시 30분`)의 요일 산술만으로 날짜를 찍었는데, 그러면 **그 주가 휴방이어도 그 날짜를 다음 방송으로 잡는다**. 2026-09-18 수집에서 오감쇼가 휴방인 9/22(화)를 다음 방송으로 잡아, 실제로는 9/29 방송인 itemList 프리뷰 상품(다이슨 New V8 무선청소기)에 `9/22(화) 방송상품` 라벨이 붙었다. 그래서 `rehd.resolve_next_broadcast()`가 ⑴ 편성표가 아는 '아직 시작 안 한 가장 이른 회차'를 먼저 쓰고, ⑵ 편성표에 아직 없으면 요일 산술로 후보를 잡되 **편성표는 있는데 이 프로그램 이름이 없는 날(=휴방)은 다음 주로 넘긴다**(`off_air_state()`. 편성표 자체가 없는 날은 판단 근거가 없으니 그대로 쓴다). `pgm-comm`이 휴방인 날을 물고 있어도 편성표가 다른 회차를 알면 편성표를 믿는다
- 스크래퍼가 날짜를 잘못 찍어 월 누적에 남은 **유령 회차는 `build_celeb_history`가 편성표로 지운다** — `before` 단계(아직 방송 전) + 새 수집분에 없는(stale) 회차 중 편성표가 휴방이라고 하는 것만. 편성 미공개인 날은 안 건드린다(판단 근거가 없으면 보존이 원칙)
...
- 회차가 묶이면 옛 회차 항목이 월 누적에 유령으로 남으므로, `build_celeb_history`는 **새 회차의 방송 구간 안에 들어가는(= 흡수된) 옛 회차를 지운다**(구간은 상품의 `segment_time`에서 읽는다). "새 수집분에 없으면 지운다"로 하면 안 된다 — 수집이 한 회차를 일시적으로 놓치면 멀쩡한 기록이 날아간다(2026-09-08 오감쇼 08:15 기록이 실제로 그렇게 지워졌다). 정정 창·확정 회차는 절대 안 건드린다
- **HD 편성표의 `brodTitl`은 시점에 따라 통째로 비어서 온다**(2026-09-04 낮 수집에서 9/8 편성 전체의 이름이 사라졌다). 그래서 ⑴ `hd_scraper`가 한 번 확인된 `pgm`을 다음 수집에서 비어 오면 물려주고, ⑵ `rehd`는 tv-list에 이름이 없으면 로컬 편성이 아는 회차 시각으로 tv-list 상품을 고른다(그마저 없을 때만 pgm-comm 시간대 폴백)
