...
- **신규(New) 판정 — `recompute_new_flags()`** — 매 실행 마지막에 주차 파일 전체를 훑어 각 프로그램에 `isNew`/`newReason`을 다시 계산(멱등, 값이 바뀐 파일만 재기록). 우선순위는 ① `episode == 1`(→ `newReason: "episode"`), ② 회차를 못 얻었을 때의 폴백으로 **보유한 모든 과거 주차를 통틀어** 처음 등장한 프로그램(→ `newReason: "firstSeen"`). 직전 주만 비교하면 컷오프(드라마 5%/예능 1%) 근처에서 출렁이다 돌아온 프로그램이 신규로 오탐되기 때문. 이력이 부족한 초기 구간(주차 파일 2개 미만이거나 누적 프로그램이 20건 미만/이번 주차의 절반 미만)에는 ②를 적용하지 않음
