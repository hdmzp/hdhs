...
- **신규 론칭 배지**: `CEL_LAUNCH_INFO`에 론칭일을 등록한 프로그램은 슬라이서 버튼과 밴드 헤더 제목에 배지가 붙는다. 론칭일 전에는 `론칭예정`(파랑), 론칭 후 `CEL_NEW_BADGE_DAYS`(60일) 동안 `NEW`(빨강), 그 뒤엔 자동으로 사라진다(날짜 기준 판단이라 배지를 지우러 다시 손댈 필요 없음). 현재 등록: `CJ_KSY.json`(김신영이 산다, 2026-08-18 론칭)
