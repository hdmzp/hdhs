...
- **첫 방송일 조회 — `lookup_first_air_dates()`** — 각 카드의 `link`(프로그램 네이버 정보 페이지)에 방영 기간(`2026.07.03. ~`)이 표기되므로, 수집 후 아직 모르는 프로그램만 상세 페이지를 방문해 첫 방송일을 뽑아 `data/dramavariety/first_air_dates.json`에 캐시(프로그램당 1회, 실행당 최대 40건, 실패 건은 7일 후 재시도)
- **신규(New) 판정 — `recompute_new_flags()`** — 매 실행 마지막에 주차 파일 전체를 훑어 각 프로그램에 `isNew`/`newReason`을 다시 계산(멱등, 값이 바뀐 파일만 재기록). 우선순위는 ① `episode == 1`(→ `newReason: "episode"`), ② 캐시된 첫 방송일이 그 주(월~일)에 속함(→ `newReason: "firstAir"`). ⚠️ "이력에 처음 등장" 방식은 쓰지 않는다 — 시청률 컷오프(드라마 5%/예능 1%) 미달로 초반 몇 주 데이터에 안 잡히다가 뒤늦게 컷오프를 넘은 프로그램(예: 전현무계획4 — 1회는 7/3인데 7/17에야 1.3%로 처음 등장)이 신규로 오탐되기 때문
- **컷오프 미만 신규 안내 — `newBelowCutoff`** — 컷오프 미만 카드도 수집은 해서 주차 파일의 `newBelowCutoff` 배열에 담아둔다(표에는 안 실림). `recompute_new_flags()`가 같은 기준으로 판정해, 신규 확정이면 `isNew`를 붙이고, 첫 방송일이 그 주 밖으로 확인되면 제거하고, 주중에 컷오프를 넘어 `programs`로 올라간 프로그램도 제거(그리드 배지가 대신 표시). 첫 방송일 미확인 항목은 판정 보류 상태로 유지
- 프론트(`index.html`)는 `isNew`가 붙은 카드 제목 옆에 작은 `New` 배지를 그리고(툴팁으로 1회 확인/첫 방송일 판정을 구분), `newBelowCutoff` 중 `isNew`인 프로그램은 표 아래 "신규 편성 — 시청률 기준 미만" 목록으로 안내(클릭 시 네이버 검색결과 이동)
